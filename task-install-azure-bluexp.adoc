...
[.lead]
Set up your networking, prepare Azure permissions, and then create a Connector in Azure from BlueXP.
...
Set up your networking so the Connector can manage resources and processes within your hybrid cloud environment. For example, you need a virtual network that has outbound internet access and network connections to target networks.

[role="quick-margin-para"]
<<Set up networking,View the networking requirements>>.

.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-2.png[Two] Set up Azure permissions

[role="quick-margin-para"]
Create a custom role and then apply it to your Azure account or to an Azure AD service principal. BlueXP authenticates with Azure and uses these permissions to create the Connector instance on your behalf.

[role="quick-margin-para"]
<<Set up Azure permissions,Follow step-by-step instructions>>.
...
[role="quick-margin-para"]
<<Create the Connector,Follow step-by-step instructions>>.

== Set up networking

Ensure that the network location where you plan to install the Connector supports the following requirements.
...
+
<<Set up an authentication method,Learn how to obtain these values for a service principal>>.
